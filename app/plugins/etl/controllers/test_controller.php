...
class TestController extends EtlAppController {
  ...
  public function index () {
    ...
    $data = $this->Admission->find('all', array(
      'conditions' => array('Admission.ResidentID' => 'ALF E'),
  ...
  public function structureResidentData ($data) {
    ...
    // debug ($data['Admission']);  debug ($data['ArMasterReference']); die;
  ...
  public function structureAssessmentData ($data) {
    ...
    foreach ($data['AssessmentControl'] as $key => $value) {
      ...
      // look for past assessment to utilize
      ...
      foreach ($section as $key => $value) {
        $section[$key]['id'] = $assessment_id;
        $section[$key]['assessment_id'] = $assessment_id;
        $section[$key]['validated'] = 1;
        $this->Assessment->{$key}->create();
        $this->Assessment->{$key}->save($section[$key], false);
      }

      $this->RugCache->update_cache($this->Assessment->id);
