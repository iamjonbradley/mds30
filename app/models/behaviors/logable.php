...
class LogableBehavior extends ModelBehavior 
{
  ...
  public $settings = array();
    ...
    function _saveLog(&$Model, $logData, $title = null) {  
    	...
    	if (isset($Model->data[$Model->alias]['assessment_id'])) {
    		$logData['Log']['assessment_id'] = $Model->data[$Model->alias]['assessment_id'];
    	}
      

			
      if ($Model->alias == 'Assessment') {
				if (!isset($Model->data[$Model->alias]['id'])) $Model->data[$Model->alias]['id'] = '';
        $logData['Log']['assessment_id'] = $Model->data[$Model->alias]['id'];
			}
			
    	if (isset($this->Log->_schema['ip_address'])) {		
				if (!isset($_SERVER['REMOTE_ADDR'])) $_SERVER['REMOTE_ADDR'] = '127.0.0.1';
    		$logData['Log']['ip_address'] = $_SERVER['REMOTE_ADDR'];
    	}
    	...
    	if (isset($this->Log->_schema[$this->settings[$Model->alias]['userKey']]) && isset($this->user[$this->UserModel->alias])) {
    		$logData['Log'][$this->settings[$Model->alias]['userKey']] = $this->user[$this->UserModel->alias][$this->UserModel->primaryKey];
    	}  	
    	
        if (isset($this->Log->_schema['description']) && isset($this->user[$this->UserModel->alias])) {
