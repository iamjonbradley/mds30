 ...
 */
	...
	function getSafeTreeList (&$Model, $id) {

		$this->Model = ClassRegistry::init($Model->name);

		$tree = $this->Model->generatetreelist(); 

		if ($this->Model->name == 'Facility') {
			$this->Session = new SessionComponent();
			$id = $this->Session->read('Auth.Facility.id');
		}
		...
		for ($i = 0; $i < $count; $i++) {
			unset($tree[$path[$i][$this->Model->name]['id']]);
		}
		...
		if ($this->Model->name == 'Facility') {
			...
			$fac = $this->Model->find('first', array(
				'conditions' => array('Facility.id' => $id, 'Facility.status' => 1),
				'fields' => array('Facility.parent_id', 'Facility.name')
			));

			if ($id != 1)
				$conditions[$this->Model->name .'.parent_id'] = $id;

			$conditions[$this->Model->name .'.status'] = 1;
		}
		...
		if (empty($list))
			$tree = array($id => $fac['Facility']['name']);
 ...
 */
	function generatetreelist(&$Model, $conditions = null, $keyPath = null, $valuePath = null, $spacer = ' - ', $recursive = null) {
		$overrideRecursive = $recursive;
		extract($this->settings[$Model->alias]);
		if (!is_null($overrideRecursive)) {
			$recursive = $overrideRecursive;
