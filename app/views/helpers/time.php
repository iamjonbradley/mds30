  ...
  function timeAgoInWords($dateTime, $options = array()) {
    ...
    if ($diff > abs($now - $this->fromString($end))) {
      ...
      if ($years > 0) {
        ...
        // minutes only
        $relativeDate .= ($relativeDate ? ', ' : '') . $minutes . ' ' . __n('min', 'mins', $minutes, true);
      } else {
        // seconds only
        $relativeDate .= ($relativeDate ? ', ' : '') . $seconds . ' ' . __n('sec', 'sec', $seconds, true);
      }

      if (!$backwards) {
        $relativeDate = sprintf(__('%s', true), $relativeDate);
      }
    }
    return $relativeDate;
  }
